...
from common.structs import SearchQuery
...
class SchemePlanState(TypedDict):
    ...
    date: datetime
    location: str
    scene: Scene  # 分配给每个方案的场景
    scheme: Scheme
    schemes_list: Annotated[list[Scheme], operator.add]  # 收集汇总所有的方案,Final key we duplicate in outer state for Send() API
...
class SceneState(TypedDict):
    ...
    source_strs: list[str]
    schemes_list: Annotated[list[Scheme], operator.add]  # Send() API key,收集汇总所有的方案
...
class SchemeStateOutPut(TypedDict):
    ...
    # queries: Queries
    # source_strs: List[str]
    schemes_list: Annotated[list[Scheme], operator.add]
